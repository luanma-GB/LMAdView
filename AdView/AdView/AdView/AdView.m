...
+(id)adScrollViewWithFrame:(CGRect)frame imageModels:(NSArray *)imageModels placeHoderImageName:(NSString *)imageName pageControlShowStyle:(UIPageControlShowStyle)PageControlShowStyle
{
    AdView * adView = [[AdView alloc]initWithFrame:frame];
    adView.placeHoldImage = [UIImage imageNamed:imageName];
    [adView setimageModels:imageModels];
    [adView setPageControlShowStyle:PageControlShowStyle];
...
-(void)setimageModels : (NSArray *) imageModels
{
    ...
    self.adTitleArray=titleArray;
    
    [self setAdTitleArray:_adTitleArray withShowStyle:AdTitleShowStyleRight];
    [self setimageLinkURL:self.imageLinkURL];

}
#pragma mark - 设置广告所使用的图片(名字)
- (void)setimageLinkURL:(NSMutableArray *)imageLinkURL
{
